...
class CaseAnalysisResponse(BaseModel):
    ...
    top_sections: Optional[List[Dict[str, Any]]] = None
...
async def analyze_case(request: CaseAnalysisRequest):
    ...
    if "error" in llm_response:
        analysis_report = f"## Analysis Service Status\n\n**Service Notice:** {llm_response['error']}\n\nI have retrieved authoritative local precedents and statutory matches below to ensure you have a definitive baseline for your case assessment. These local results are treated as the 'Ground Truth' for this inquiry."
        recommended_actions = ["Check local legal precedents below", "Verify statutory citations"]
        risk_score = 0
        expected_direction = "Analysis synchronization in progress..."
        comparison_rows = []
    else:
    ...
    try:
        ...
        json_path = os.path.join(settings.BASE_DIR, "server", "data", "MatchedCase", "IndicLegalQA Dataset_10K.json")
        if not os.path.exists(json_path):
             json_path = os.path.join(settings.BASE_DIR, "data", "MatchedCase", "IndicLegalQA Dataset_10K.json")
            ...
            with open(json_path, 'r', encoding='utf-8') as f:
                ...
                for score, entry in matches[:3]:
                    matched_json_cases.append({
                        ...
                        "question": entry.get('question', 'Similar legal question addressed.'),
                        "ratio": entry.get('answer', 'Ratio decidendi details from the case.')
                    })
    except Exception as e:
        print(f"JSON matching error: {e}")

    # 5. FETCH TOP SECTIONS from CSV based on keywords
    top_sections_matches = []
    try:
        from app.core.config import settings
        csv_path = os.path.join(settings.BASE_DIR, "server", "data", "TopJudgement", "top_sections.csv")
        if not os.path.exists(csv_path):
             csv_path = os.path.join(settings.BASE_DIR, "data", "TopJudgement", "top_sections.csv")
             
        if os.path.exists(csv_path):
            search_keywords = (request.issues + " " + (request.sections or "")).lower().split()
            with open(csv_path, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if score > 0:
                        top_sections_matches.append((score, row))
                
                top_sections_matches.sort(key=lambda x: x[0], reverse=True)
                top_sections_matches = [{
                    "title": r.get("title"),
                    "docsource": r.get("docsource"),
                    "url": r.get("url"),
                    "rank": r.get("rank")
                } for score, r in top_sections_matches[:4]]
    except Exception as e:
        print(f"Sections search error: {e}")
    ...
    if not all_matched:
        for i, p in enumerate(local_precedents[:3]):
             all_matched.append({
                 ...
                 "year": "N/A",
                 "matchScore": 85,
                 "question": "Matches facts/issues in local analysis.",
                 "ratio": p['answer']
             })
    ...
    return CaseAnalysisResponse(
        ...
        top_sections=top_sections_matches,
...
async def get_top_judgments():
    ...
    try:
        from app.core.config import settings
        # Resolve data path using central Settings for consistency across all CWDs
        csv_path = os.path.join(settings.BASE_DIR, "server", "data", "TopJudgement", "top_judgments.csv")
        if not os.path.exists(csv_path):
             csv_path = os.path.join(settings.BASE_DIR, "data", "TopJudgement", "top_judgments.csv")
...
@router.get("/judgment/{case_id}")
async def get_judgment_details(case_id: str):
    """Retrieves structured judgment details from JSON files with robustness for titles/dates."""
    try:
        from app.core.config import settings
        ...
        def to_slug(text: str) -> str:
            ...
            return clean.lower().strip().replace(" ", "_").replace("__", "_")
            
        slug_full = to_slug(case_id)
        # Handle "Case Name on Date" pattern (very common in Indian Kanoon/CSV)
        slug_short = to_slug(case_id.split(" on ")[0]) if " on " in case_id else slug_full
        
        possible_paths = [
            os.path.join(settings.BASE_DIR, "server", "data", "JudgmentDetails", f"{slug_full}.json"),
            os.path.join(settings.BASE_DIR, "data", "JudgmentDetails", f"{slug_full}.json"),
            os.path.join(settings.BASE_DIR, "server", "data", "JudgmentDetails", f"{slug_short}.json"),
            os.path.join(settings.BASE_DIR, "data", "JudgmentDetails", f"{slug_short}.json"),
        ]
        ...
        # FALLBACK: Directory Scan for Partial Match
        if not json_path:
            details_dir = os.path.join(settings.BASE_DIR, "server", "data", "JudgmentDetails")
            if not os.path.exists(details_dir):
                details_dir = os.path.join(settings.BASE_DIR, "data", "JudgmentDetails")
            
            if os.path.exists(details_dir):
                files = os.listdir(details_dir)
                # Try to find a file that starts with our slug or contains the core name
                for f in files:
                    clean_f = f.lower()
                    if clean_f.startswith(slug_short) or slug_short in clean_f:
                        json_path = os.path.join(details_dir, f)
                        break

        if not json_path:
            raise HTTPException(status_code=404, detail=f"Judgment details not found for '{case_id}'. (Checked: {slug_full}, {slug_short})")
...
async def get_top_sections():
    """Reads and returns statutes/sections from top_sections.csv."""
    sections = []
    try:
        from app.core.config import settings
        csv_path = os.path.join(settings.BASE_DIR, "server", "data", "TopJudgement", "top_sections.csv")
            ...
            with open(csv_path, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    sections.append({
                        ...
                        "docsource": row.get("docsource"),
                        "url": row.get("url"),
                        "title": row.get("title")
                    })
        return {"results": sections[:100], "total": len(sections)} # Return top 100 for now
    except Exception as e:
        print(f"Sections read error: {e}")
