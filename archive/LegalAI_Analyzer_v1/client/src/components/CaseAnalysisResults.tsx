...
import { AlertTriangle, CheckCircle, FileText, TrendingUp, TrendingDown, Minus, Scale, BookOpen, Quote, Loader2 } from 'lucide-react';
...
export default function CaseAnalysisResults({ data }: CaseAnalysisResultsProps) {
  ...
  const handleFullJudgmentClick = async (caseName: string) => {
    setIsLoadingJudgment(true);
    try {
      const details = await api.getJudgmentDetails(caseName);
      setSelectedJudgment(details);
    } catch (error) {
      console.error("Failed to fetch judgment details:", error);
      alert("Could not retrieve full judgment details at this time.");
    } finally {
            ...
            <div key={caseItem.id} className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-indigo-300 hover:shadow-2xl hover:shadow-indigo-50/50 transition-all group flex flex-col h-full">
              ...
              <div className="mt-5 pt-5 border-t border-gray-100 flex items-center justify-between">
                 <span className="text-xs font-bold text-gray-400">{caseItem.court} • {caseItem.year}</span>
                  <button 
                    onClick={() => handleFullJudgmentClick(caseItem.name)}
                    disabled={isLoadingJudgment}
                    className="text-xs font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-1 disabled:opacity-50"
                  >
                    {isLoadingJudgment ? (
                      <>
